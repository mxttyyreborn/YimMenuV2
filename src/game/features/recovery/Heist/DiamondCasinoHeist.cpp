	...
	namespace DiamondCasinoHeist
	{
		...
		virutal void OnCall() override
		{
			if (auto thread = Scripts::FindScriptThread("fm_mission_controller"_J))
			{
				...
				*ScriptLocal(thread, 63640).As<int*>() = 8;
			}
		}
	};
		...
		static SoloMantrap _DiamondCasinoHeistSkipDrilling{"diamondcasinoheistsolomantrap", "Solo Mantrap", "Skips card swiping process"};
